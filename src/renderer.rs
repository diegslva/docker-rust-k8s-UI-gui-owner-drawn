...
impl GpuState {
    pub async fn new(window: Arc<Window>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            pipeline_2d, pipeline_2d_prim, pipeline_3d_opaque, pipeline_3d_alpha,
            camera_bind_group_layout, camera_buffer, camera_bind_group,
            prim_vert_buf, prim_idx_buf,
            depth_texture, size,
    ...
    fn build_pipeline_2d_prim(device: &Device, config: &SurfaceConfiguration) -> RenderPipeline {
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("layout_2d_prim"), bind_group_layouts: &[], push_constant_ranges: &[],
        });
        ...
        device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("pipeline_2d_prim"), layout: Some(&layout),
            vertex: wgpu::VertexState {
                module: &shader, entry_point: Some("vs_main"),
                buffers: &[vert_layout], compilation_options: Default::default(),
            },
            fragment: Some(wgpu::FragmentState {
                module: &shader, entry_point: Some("fs_main"),
                targets: &[Some(wgpu::ColorTargetState {
