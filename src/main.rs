...
impl App {
        ...
        std::thread::spawn(move || {
            ...
            let meshes: Vec<LoadedMesh> = defs
                ...
                .filter_map(|(path, tint, alpha)| {
                    Mesh::from_obj(&device, path).ok().map(|m| LoadedMesh {
                        mesh: m,
                        tint: *tint,
                        alpha: *alpha,
                    })
    ...
    /// Retorna true se a posição (mx, my) está dentro da zona da menu bar ou dropdown aberto.
    /// Usado para: (a) decidir o cursor (seta vs grab), (b) bloquear camera drag.
    fn is_menu_zone(&self, my: f64) -> bool {
    ...
    fn build_labels(&mut self, size: PhysicalSize<u32>) {
        ...
        let case_val = if self.scan.case_id.is_empty() {
            "—".to_string()
        } else {
            self.scan.case_id.clone()
        };
        let dataset_val = if self.scan.dataset.is_empty() {
            "—".to_string()
        } else {
            self.scan.dataset.clone()
        };
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
                                    ...
                                    (1, n) if n >= 3 => {
                                            ...
                                            && case_idx != self.current_case
                                        {
                                            ...
                                            std::thread::spawn(move || {
                                                ...
                                                let defs = [
                                                    (
                                                        format!("{}/tumor_et.obj", case_dir),
                                                        ET_COLOR,
                                                        1.0_f32,
                                                    ),
                                                    (
                                                        format!("{}/tumor_snfh.obj", case_dir),
                                                        SNFH_COLOR,
                                                        1.0_f32,
                                                    ),
                                                    (
                                                        format!("{}/tumor_netc.obj", case_dir),
                                                        NETC_COLOR,
                                                        1.0_f32,
                                                    ),
                                                ];
                                                let meshes: Vec<LoadedMesh> = defs
                                                    .iter()
                                                    .filter_map(|(path, tint, alpha)| {
                                                        Mesh::from_obj(&device, path).ok().map(
                                                            |m| LoadedMesh {
                                                                mesh: m,
                                                                tint: *tint,
                                                                alpha: *alpha,
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                // ── Cursor: seta na menu zone, grab/grabbing na cena 3D ────────
                ...
                if !self.splash_done {
                    ...
                    if let Some(rx) = &self.splash_rx {
                        if let Ok(new_meshes) = rx.try_recv() {
                            ...
                            self.scan = ScanMeta::load(&ScanMeta::case_path(TOP_CASES[0]));
                            self.splash_rx = None;
                ...
                if let Some(path) = picked_path {
                    ...
                    std::thread::spawn(move || {
                        ...
                        if ok {
                            ...
                            let _meshes: Vec<LoadedMesh> = defs
                                .iter()
                                .filter_map(|(p, t, a)| {
                                    Mesh::from_obj(&device_bg, p).ok().map(|m| LoadedMesh {
                                        mesh: m,
                                        tint: *t,
                                        alpha: *a,
                                    })
                    ...
                    if let Ok(ok) = rx.try_recv() {
                        ...
                        if ok {
                            ...
                            let new_meshes: Vec<LoadedMesh> = {
                                defs.iter()
                                    .filter_map(|(p, t, a)| {
                                        Mesh::from_obj(&device, p).ok().map(|m| LoadedMesh {
                                            mesh: m,
                                            tint: *t,
                                            alpha: *a,
                                        })
                ...
                let meshes_arrived = if let Some(rx) = &self.loading_rx {
                    match rx.try_recv() {
                        Ok(new_meshes) => {
                            ...
                            self.scan =
                                ScanMeta::load(&ScanMeta::case_path(TOP_CASES[self.current_case]));
                            rebuild_needed = true;
